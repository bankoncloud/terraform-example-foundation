...
data "google_projects" "network_projects" {
  filter = "parent.id:${split("/", data.google_active_folder.env.name)[1]} labels.application_name=${var.vpc_type}-shared-vpc-host labels.environment=${var.environment} lifecycleState=ACTIVE"
}
...
data "google_projects" "environment_projects" {
  filter = "parent.id:${split("/", data.google_active_folder.env.name)[1]} name:*${var.project_suffix}* labels.application_name=${var.business_code}-boa-application labels.environment=${var.environment} lifecycleState=ACTIVE"
}
